 ...
 * 
 * 
 * Based on "Thinking in Java" PL, ed. IV p. 883
...
class Wheel implements ICarElement {
    ...
    public void accept(ICarElementVisitor visitor) {
        /*
         * accept(ICarElementVisitor) in Wheel implements accept(ICarElementVisitor) in ICarElement, so the call to
         * accept is bound at run time. This can be considered the first dispatch. However, the decision to call
         * visit(Wheel) (as opposed to visit(Engine) etc.) can be made during compile time since 'this' is known at
         * compile time to be a Wheel. Moreover, each implementation of ICarElementVisitor implements the visit(Wheel),
